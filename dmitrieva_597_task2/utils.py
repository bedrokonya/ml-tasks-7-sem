...
def WintersLinearTrendExponentialSmoothing(x, h, Params):
    ...
    s = [np.NaN] * p # initalize seasonality values (it must be vector of lenth p)\    l= np.NaN
    ...
    for cntr in range(T):
        if not math.isnan(x[cntr]):
            ...
            b = beta * (l - l_old) + (1 - beta) * b # recurrent smoothing of trend
